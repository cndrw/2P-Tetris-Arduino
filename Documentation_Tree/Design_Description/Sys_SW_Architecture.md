...
```
Nach der `holdTime` Berechnung wird die Variable `top` berechnet, die letztendlich den Wert im Output-Compare-Register annehmen wird. Der letzte Schritt in der Routine ist es den `currentNote` Zähler um zwei zu erhöhen und eine if-Abrage zu tätigen, welche falls das Ende des Liedes erreicht ist, den `currentNote` Zähler zurücksetzt, damit das Lied in einer Dauerschleife läuft.
